...
export class UserListComponent extends LitElement {
  ...
  initListners() {
    ...
    window["PubSub"].subscribe(
      "updateUserRecord",
      (_eventName: string, dataObj: object) => this.updateUserRecord(dataObj)
    );
  }
  ...
  editUserRecord(event: any) {
    ...
    if (index !== -1) {
      window["PubSub"].publish("updateUserRecord", this.tableData[index], index);
    }
  }

...
export class UserListComponent extends LitElement {
  ...
  initListners() {
    ...
    window["PubSub"].subscribe(
      "updateUserRecord",
      (_eventName: string, dataObj: object, index: number) =>
        this.updateUserRecord(dataObj, index)
    );
  }
  ...
  addUserRecord(dataObj: any) {
    const index = this.tableData.findIndex((f: any) => f.name === dataObj.name);
    if (index === -1) {
      this.tableData.push(dataObj);
      window["PubSub"].publish("addUserRecordResponse", {
        type: "success",
      ...
      window["PubSub"].publish("addUserRecordResponse", {
        type: "error",
        message: `Error while creating user. Unable to create user. A user with name ${dataObj.name} already exist.`
      });
      return;
    }
  }

  updateUserRecord(dataObj: any, index: number) {
    try {
      this.tableData[index] = dataObj;
      window["PubSub"].publish("updateUserRecordResponse", {
        type: "success",
      ...
      window["PubSub"].publish("addUserRecordResponse", {
        type: "error",
        message: `Error while updating user. ${e}`
      });
    }
  }

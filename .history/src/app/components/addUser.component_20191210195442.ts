...
export class AddUserComponent extends LitElement {
  ...
  addUserRecord(event: any) {
    ...
    if (!this.userObj.name || !this.userObj.age || !this.userObj.salary) {
      this.showMessage("error", "Invalid form");
      return;
    }
  ...
  updateUserRecord(event: any) {
    ...
    window["PubSub"].publish("updateUserRecord", this.userObj, this.index);
    window["PubSub"].subscribeOnce(
  ...
  showMessage(type: string, message: string) {
    const id = type === "success" ? "success-alert" : "error-alert";
    const alertMessage = this.shadowRoot.getElementById(id);
